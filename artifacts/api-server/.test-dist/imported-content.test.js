// src/lib/imported-content.test.ts
import assert from "node:assert/strict";
import test from "node:test";

// src/lib/imported-content.ts
var INTERNAL_IMPORT_EXCERPT = /^Imported listing from \S+\. Review and edit before republishing\.?$/i;
var BLOCKED_IMPORT_TERMS = /(?:^|[^a-z0-9])(?:porn(?:ography|ographic)?|sex(?:ual|y)?)(?=$|[^a-z0-9])/i;
var replaceVisitorTerms = (value) => value.replace(/\bwatch\b/gi, "See").replace(/\bdownload\b/gi, "Get");
var containsBlockedImportTerms = (...values) => values.some((value) => {
...
var buildImportedExcerpt = (title) => `Explore ${replaceVisitorTerms(title)} and view its release details and authorized source information.`;
var cleanImportedExcerpt = (excerpt, title) => {
  const cleanedExcerpt = INTERNAL_IMPORT_EXCERPT.test(excerpt.trim()) ? buildImportedExcerpt(title) : excerpt;
