...
**How to apply:** For any new outbound importer request, validate every resolved address, reject private/reserved IPv4 and IPv6 ranges, pin the connection lookup to a validated address, and repeat the process for every redirect hop. Normalize IPv6 before classification so expanded IPv4-mapped forms cannot bypass prefix checks. Support both single-address and `all: true` lookup callback shapes.