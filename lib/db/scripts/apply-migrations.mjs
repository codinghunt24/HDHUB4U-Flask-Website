...
try {
  ...
  const internalMediaMigrationName = "20260827_store_internal_post_images";
  const { rowCount: internalMediaMigrationCount } = await client.query(
    "SELECT 1 FROM app_migrations WHERE name = $1",
    [internalMediaMigrationName],
  );

  if (internalMediaMigrationCount === 0) {
    // Do not delete legacy URLs here. Public serialization refuses to expose
    // them, while the existing bounded refresh/enrichment operations need
    // those values as a safe backfill source before any later cleanup.
    await client.query(
      "INSERT INTO app_migrations (name) VALUES ($1)",
      [internalMediaMigrationName],
    );
    console.log(`Applied database migration: ${internalMediaMigrationName}`);
  } else {
    console.log(
      `Database migration already applied: ${internalMediaMigrationName}`,
    );
  }

  const adsterraMigrationName = "20260828_add_adsterra_settings";
  const { rowCount: adsterraMigrationCount } = await client.query(
    "SELECT 1 FROM app_migrations WHERE name = $1",
    [adsterraMigrationName],
  );

  if (adsterraMigrationCount === 0) {
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_enabled boolean NOT NULL DEFAULT false",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_top_enabled boolean NOT NULL DEFAULT true",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_top_code text",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_content_enabled boolean NOT NULL DEFAULT true",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_content_code text",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_sidebar_enabled boolean NOT NULL DEFAULT true",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_sidebar_code text",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_footer_enabled boolean NOT NULL DEFAULT true",
    );
    await client.query(
      "ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS adsterra_footer_code text",
    );
    await client.query(
      "INSERT INTO app_migrations (name) VALUES ($1)",
      [adsterraMigrationName],
    );
    console.log(`Applied database migration: ${adsterraMigrationName}`);
  } else {
    console.log(`Database migration already applied: ${adsterraMigrationName}`);
  }
